...
RSpec.describe Order do
  ...
  describe 'instance methods' do
    before :each do
      ...
      @address = @user.addresses.create(nickname: "work", address: '123 Main St', city: 'Marianna', state: 'Colorado', zip: 34567)
  ...
  describe 'class methods' do
    before :each do
      ...
      address = @user.addresses.create(nickname: "work", address: '123 Main St', city: 'Marianna', state: 'Colorado', zip: 34567)
      @order_1 = @user.orders.create!(status: 1, address_id: address.id)
      @order_2 = @user.orders.create!(status: 0, address_id: address.id)
      @order_3 = @user.orders.create!(status: 3, address_id: address.id)
      @order_4 = @user.orders.create!(status: 2, address_id: address.id)
    end
